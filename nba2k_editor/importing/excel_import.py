...
from ..models.schema import BUFFER_CODEC_FALLBACK, BufferDecodeConfig, decode_field_value_from_buffer
...
def _build_player_snapshot(
    ...
    with timed("excel_import.build_player_snapshot"):
        if not entities:
            return None
        player_stride = model.offset_state.player_stride
        if player_stride <= 0:
            return None
...
def _decode_field_value_from_record(
    ...
    *,
    ...
    if record is None:
        return _FALLBACK
    value = decode_field_value_from_buffer(
        meta,
        record,
        config=BufferDecodeConfig(
            ...
            name_max_chars=model.offset_state.name_max_chars,
            clamp_badges=False,
            clamp_height=False,
            format_hex_value=model._format_hex_value,
            team_pointer_to_display_name=model._team_pointer_to_display_name,
        ),
    )
...
def _resolve_version_key(raw: dict) -> str | None:
    ...
    if not isinstance(versions, dict) or not versions:
        return None
    target = offsets_mod.get_current_target() or offsets_mod.MODULE_NAME
    version_hint = None
