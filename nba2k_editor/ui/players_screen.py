...
from .theme import TEXT_ACCENT, TEXT_HEADING, TEXT_SECONDARY, to_rgba
    ...
    with dpg.child_window(tag="screen_players", parent=app.content_root, autosize_x=True, autosize_y=True, show=False) as tag:
        ...
        with dpg.group(horizontal=True):
            ...
            app.player_search_input_tag = dpg.add_input_text(
                hint="Search players.",
                width=220,
                callback=lambda _sender, value: _on_search_changed(app, value),
            )
            dpg.add_button(
                label="Refresh",
                callback=lambda: app._start_scan(),
                width=90,
            )
        ...
        with dpg.group(horizontal=True):
            ...
            app.scan_status_text_tag = dpg.add_text(app.scan_status_var.get(), color=to_rgba(TEXT_SECONDARY))
...
def _build_player_detail_panel(app):
    app.player_name_text = dpg.add_text(app.player_name_var.get(), bullet=False, color=to_rgba(TEXT_HEADING))
    app.player_ovr_text = dpg.add_text(app.player_ovr_var.get(), color=to_rgba(TEXT_ACCENT))
    dpg.add_spacer(height=6)
    with dpg.table(header_row=False, policy=dpg.mvTable_SizingStretchProp, row_background=False, resizable=False):
        ...
        for label, default in labels_defaults:
            with dpg.table_row():
                dpg.add_text(label)
                val_tag = dpg.add_text(default, color=to_rgba(TEXT_SECONDARY))
