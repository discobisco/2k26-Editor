...
class BatchEditWindow:
    ...
    def _apply_changes(self) -> None:
        ...
        if self.model.external_loaded or not self.model.mem.open_process():
            self.app.show_warning("Batch Edit", "NBA 2K is not running or roster is external; cannot apply.")
            return
        player_base = self.model._resolve_player_base_ptr()
        ...
        player_stride = self.model.offset_state.player_stride
        if kind == "int":
            ...
            for player in targets:
                ...
                if ok:
                    total_changed += 1
        self.app.show_info("Batch Edit", f"Applied value to {total_changed} player(s).")
        try:
    ...
    def _reset_core_fields(self) -> None:
        ...
        if self.model.external_loaded:
            self.app.show_warning("Batch Edit", "NBA 2K roster is loaded from external files. Cannot apply changes.")
            return
        if not self.model.mem.hproc and not self.model.mem.open_process():
            self.app.show_warning("Batch Edit", "NBA 2K is not running. Cannot apply changes.")
            return
        selected_teams = self._selected_teams()
        cached_players = list(self.model.players or [])
        if not cached_players:
        ...
        for player in players_to_update:
            ...
            for spec, value in post_actions:
                ...
                if ok:
                    total_updated += 1
        self.app.show_info("Batch Edit", f"Reset core fields for {len(players_to_update)} player(s).")
        try:
            self.model.refresh_players()
        except Exception:
            pass
