...
from .theme import TEXT_PRIMARY, TEXT_SECONDARY, to_rgba
    ...
    def _build_ui(self) -> None:
        with dpg.window(label=self.title, tag=self.window_tag, modal=True, no_collapse=True, width=760, height=520):
            ...
            if self.missing_players:
                ...
                dpg.add_text(self.missing_label, color=to_rgba(TEXT_PRIMARY))
                with dpg.child_window(height=220, border=True):
                    for name in self.missing_players:
                        with dpg.group(horizontal=True):
                            dpg.add_text(name, color=to_rgba(TEXT_PRIMARY))
                            ...
                            if isinstance(score_val, (int, float)):
                                ...
                                dpg.add_text(pct_text, color=to_rgba(TEXT_SECONDARY))
...
__all__ = ["ImportSummaryDialog", "TeamSelectionDialog"]
