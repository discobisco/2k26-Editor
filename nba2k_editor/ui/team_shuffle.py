...
class TeamShuffleWindow:
    ...
    def _shuffle_selected(self) -> None:
        ...
        total_assigned = 0
        offset_state = self.model.offset_state
        if live_mode:
            ...
            for idx, name in self.model.team_list:
                if name and "free" in name.lower():
                    free_ptr = team_base + idx * offset_state.team_record_size
                    break
            if free_ptr is None:
            ...
            for idx, name in self.model.team_list:
                if name in selected:
                    team_ptrs[name] = team_base + idx * offset_state.team_record_size
            for player in players_to_pool:
                try:
                    p_addr = player_base + player.index * offset_state.player_stride
                    self.model.mem.write_pointer(p_addr + offset_state.off_team_ptr, free_ptr)
            ...
            for team in selected:
                ...
                for _ in range(self.MAX_ROSTER_SIZE):
                    ...
                    try:
                        p_addr = player_base + player.index * offset_state.player_stride
                        self.model.mem.write_pointer(p_addr + offset_state.off_team_ptr, ptr)
                        player.team = team
        ...
        self.app.show_info("Shuffle Teams", f"Shuffle complete. {total_assigned} players reassigned. Remaining players are Free Agents.")
