...
from ..core.config import HOOK_TARGETS, MODULE_NAME
...
class GameMemory:
    ...
    def detect_running_module_name(preferred_module: str | None = None) -> str | None:
        """Return a supported running executable name, preferring the requested module."""
        supported = [str(exe).strip() for _label, exe in HOOK_TARGETS if str(exe).strip()]
        if not supported:
            return None
        preferred = str(preferred_module or "").strip().lower()
        ordered = []
        if preferred:
            ordered.append(preferred)
        for exe in reversed(supported):
            exe_lower = exe.lower()
            if exe_lower not in ordered:
                ordered.append(exe_lower)
        supported_set = set(ordered)
        found: dict[str, str] = {}
        try:
            ...
            for proc in psutil.process_iter(["name"]):
                ...
                if name_lower in supported_set and name_lower not in found:
                    found[name_lower] = name
        except Exception:
            pass
        if found:
            for name_lower in ordered:
                if name_lower in found:
                    return found[name_lower]
        if sys.platform != "win32" or CreateToolhelp32Snapshot is None:
            return None
